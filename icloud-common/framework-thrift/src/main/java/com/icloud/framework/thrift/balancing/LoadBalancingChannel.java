...
import com.icloud.framework.util.TZCuratorFramework;

public abstract class LoadBalancingChannel<T> implements Client<T> {
	 ...
	 * @param servcieName
	 */
	public LoadBalancingChannel(List<String> list, String servcieName) {
		...
		// }
		if (servcieName != null && servcieName.length() > 0) {
			isUseCurator = true;
	...
	public T proxy() throws Exception {
		...
		if (isUseCurator) {// 如果使用curator
			List<String> rpcList = TZCuratorFramework.getRpcAdress(serviceName);
