...
public class TestLatencyTimewindow {

	final Logger logger = LoggerFactory.getLogger(TestLatencyTimewindow.class);
...
//		final int frameTimeLen = 10;// seconds
		...
		final LatenyCounter counter = factory.createLatenyCounter("test",
					...
					for (int k = 0; k < ITEM_CNT; k++) {
						for (int j = 1; j < 10; j++) {
							String name = Thread.currentThread().getName()
									+ "n" + j;
							counter.startWatch(name);

							// 业务代码
							DateTimeUtil.sleep(random.nextInt(50));

							counter.stopWatch(name);
						}
		...
		Runnable r = new Runnable() {
			...
			public void run() {
				...
				long count = 100000;
				while (--count > 0) {
					List<Map<String, ArrayList<MutableLong>>> retall = counter
							.getAllFrameResult();
					...
					for (int i = 0; i < 10; i++) {
						for (int j = 0; j < 10; j++) {
							String name = "t" + i + "n" + j;
							long maxLatency = counter.getMaxLatency(name);
							long averageLatency = counter
									.getAverageLatency(name);
							logger.info("[TEST] " + name + " max latency = "
									+ maxLatency + " average latency = "
									+ averageLatency);
						}
					}
		...
		Map<String, ArrayList<MutableLong>> ret = counter.getLastFrameResult();
