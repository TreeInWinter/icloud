 ...
 */
public class UUBotHtmlRendererContext implements HtmlRendererContext {
	private static final Logger logger = Logger.getLogger(UUBotHtmlRendererContext.class.getName());
	 ...
	 */
	public UUBotHtmlRendererContext(HtmlPanel contextComponent) {
		super();
	 ...
	 */
	public UUBotHtmlRendererContext(HtmlPanel contextComponent, HtmlParserContext pcontext) {
	 ...
	 * @deprecated HtmlParserContext is no longer used in this class.
	 */
	public UUBotHtmlRendererContext(HtmlPanel contextComponent, HtmlParserContext pcontext, HtmlRendererContext parentRcontext) {
		super();
		this.htmlPanel = contextComponent;
	 ...
	 * Normally ony frame renderer contexts would have parents.
	 */
	public UUBotHtmlRendererContext(HtmlPanel contextComponent, HtmlRendererContext parentRcontext) {
		super();
		this.htmlPanel = contextComponent;
		this.parentRcontext = parentRcontext;
	}
		...
		new Thread() {
			public void run() {
				try {
					...
					try {
						UUBotHtmlRendererContext.this.sourceCode = null;
						...
						DocumentBuilderImpl builder = new DocumentBuilderImpl(
								pcontext, UUBotHtmlRendererContext.this);
						...
						UUBotHtmlRendererContext.this.htmlPanel.setDocument(document, UUBotHtmlRendererContext.this);
						// Now start loading.
						document.load();
						...
						logger.info("Parsed URI=[" + uri + "]: Parse elapsed: "
								...
								+ (time1 - time0) + " ms.");
						UUBotHtmlRendererContext.this.sourceCode = rin.getString("ISO-8859-1");
					} finally {
						in.close();
					}
				} catch (Exception err) {
					UUBotHtmlRendererContext.this.error(
	...
	public UserAgentContext getUserAgentContext() {
		...
		synchronized(this) {
			if(this.bcontext == null) {
				this.bcontext = new UUBotUserAgentContext();
			}
