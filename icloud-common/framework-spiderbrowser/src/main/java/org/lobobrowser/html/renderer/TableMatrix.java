...
class TableMatrix {
	 ...
	 * @param element
	 */
    public TableMatrix(HTMLTableElementImpl element, UserAgentContext pcontext, HtmlRendererContext rcontext, FrameContext frameContext, RenderableContainer tableAsContainer, RElement relement) {
	...
	public void reset(Insets insets, int availWidth, int availHeight) {
		...
		this.tableWidthLength = this.tableElement.getWidthLength(availWidth);
	...
	private void populateRows() {
		...
		ArrayList cellList = te.getDescendents(COLUMNS_FILTER);
...
//			}
		...
		for(int row = 0; row < numRows;) {
			...
			if(ac != null) {
				...
				if(colSpan > 1) {
					...
					Dimension size = ac.doCellLayout(vcActualWidth, 0, false, false);
					...
					//TODO: better height possible
					Dimension size = ac.doCellLayout(actualSize, 0, false, false);
	...
	private void determineRowSizes(int hasBorder, int cellSpacing, int availHeight) {
		HtmlLength tableHeightLength = this.tableElement.getHeightLength(availHeight);
		int tableHeight;
		SizeInfo[] rowSizes = this.rowSizes;
		int numRows = rowSizes.length;
	...
	private final void finalRender(int hasBorder, int cellSpacing) {
		...
		for(int i = 0; i < numCells; i++) {
			...
			if(size.width > totalCellWidth) {
				if(colSpan == 1) {
					if(size.width > colSizes[col].actualSize) {
						colSizes[col].actualSize = size.width;
					}
				}
				else {
					//TODO: Adjustment for colSpan > 1.
				}
			}
			if(size.height > totalCellHeight) {
				if(rowSpan == 1) {
					if(size.height > rowSizes[row].actualSize) {
						rowSizes[row].actualSize = size.height;
					}
				}
				else {
					//TODO: Adjustment for rowSpan > 1.
				}
			}			
		}
	}
