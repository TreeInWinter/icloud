...
public class RBlock extends BaseElementRenderable implements RenderableContainer, ImageObserver {
	...
	private static final LineMargin INVALID_MARGIN = new LineMargin(null, 0, 0);
	...
	private LineMargin lastLeftMargin = INVALID_MARGIN;
	private LineMargin lastRightMargin = INVALID_MARGIN;
	...
	public final void layout(int availWidth, int availHeight, boolean expandWidth, boolean expandHeight, int defaultOverflow) {
		this.layout(availWidth, availHeight, expandWidth, expandHeight, null, null, 0, defaultOverflow);
	}

	public final void layout(int availWidth, int availHeight, boolean expandWidth, boolean expandHeight, LineMargin leftMargin, LineMargin rightMargin, int tentativeY) {
		this.layout(availWidth, availHeight, expandWidth, expandHeight, leftMargin, rightMargin, tentativeY, this.defaultOverflow);
	}

	public final void layout(int availWidth, int availHeight, boolean expandWidth, boolean expandHeight, LineMargin leftMargin, LineMargin rightMargin, int tentativeY, int defaultOverflow) {
		try {
			this.doLayout(availWidth, availHeight, expandWidth, expandHeight, leftMargin, rightMargin, tentativeY, defaultOverflow);
		} finally {
			this.layoutUpTreeCanBeInvalidated = true;
...
//			this.renderStyleCanBeInvalidated = true;
	...
	public final void doLayout(int availWidth, int availHeight, boolean expandWidth, boolean expandHeight) {
		this.doLayout(availWidth, availHeight, expandWidth, expandHeight, null, null, 0, this.defaultOverflow);
	}

	public final void doLayout(int availWidth, int availHeight, boolean expandWidth, boolean expandHeight, int tentativeY) {
		this.doLayout(availWidth, availHeight, expandWidth, expandHeight, null, null, tentativeY, this.defaultOverflow);
	}
	 ...
	 * @param availHeight
	 */
	public void doLayout(int availWidth, int availHeight, boolean expandWidth, boolean expandHeight, LineMargin leftMargin, LineMargin rightMargin, int tentativeY, int defaultOverflow) {
		...
		if(!forced) {
			if(renderState != null) {
 			...
 			if(!forced) {
				if(this.lastLeftMargin == INVALID_MARGIN || this.lastRightMargin == INVALID_MARGIN) {
					forced = true;
				}
				else {
					forced = !Objects.equals(this.lastLeftMargin, leftMargin) || !Objects.equals(this.lastRightMargin, rightMargin);
				}
			}
		}
		if(forced) {
			this.forceLayout(renderState, availWidth, availHeight, expandWidth, expandHeight, leftMargin, rightMargin, tentativeY, defaultOverflow);
		}
		else {
			// nothing to do
		}
	 ...
	 * @return
	 */
	private final void forceLayout(RenderState renderState, int availWidth, int availHeight, boolean expandWidth, boolean expandHeight, LineMargin leftMargin, LineMargin rightMargin, int tentativeY, int defaultOverflow) {
		// Expected to be invoked in the GUI thread.
		...
		for(int tries = ((auto && !vscroll) ? 0 : 1); tries < 2; tries++) {
			try {
				...
				LineMargin childLeftMargin = leftMargin == null || mayScroll ? null : leftMargin.translated(tentativeY + insets.top + paddingInsets.top, insets.left + paddingInsets.left);
				LineMargin childRightMargin = rightMargin == null || mayScroll ? null : rightMargin.translated(tentativeY + insets.top + paddingInsets.top, insets.right + paddingInsets.right + (availWidth - tentativeWidth));
				bodyLayout.layout(desiredViewportWidth, desiredViewportHeight, paddingInsets, node, maxY, childLeftMargin, childRightMargin);
				break;
		...
		if(pairs != null) {
			...
			while(i.hasNext()) {
				...
				if(pair.targetParent == this) {
					bodyLayout.importDelayedPair(pair);
				}
			}
		}
	}
...
//		}
	...
	public void invalidateLayoutLocal() {
		...
		this.lastLeftMargin = INVALID_MARGIN;
		this.lastRightMargin = INVALID_MARGIN;
	}
	...
	public boolean onMouseClick(MouseEvent event, int x, int y) {
		...
		if(!HtmlController.getInstance().onMouseClick(this.modelNode)) {
			return false;
		}
	...
	public boolean onMouseReleased(MouseEvent event, int x, int y) {
		...
		if(!HtmlController.getInstance().onMouseUp(this.modelNode)) {
			return false;
		}
		if(this.backgroundColor != null) {
			return false;
