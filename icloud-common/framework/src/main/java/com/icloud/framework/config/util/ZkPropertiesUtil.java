...
public class ZkPropertiesUtil {
	private static Logger logger = LoggerFactory
			.getLogger(ZkPropertiesUtil.class);
	...
	public static byte[] getValueFromZk(String zkPath, boolean isNeedCached) {
		...
		if (isNeedCached && zkMap.containsKey(zkPath)) {
			...
			try {
				...
				Stat stat = TopsCuratorFramework.getInstance().checkNodePath(
						zkPath, dataWatcher);
				if (stat != null) {// 节点存在，去获取数据
					byte[] data = TopsCuratorFramework.getInstance().getData(
							zkPath);
		...
		public void process(WatchedEvent event) throws Exception {
			try {
				...
				if (event.getType() == EventType.NodeDataChanged
					...
					try {
						byte[] data = TopsCuratorFramework.getInstance()
								.getData(path);
